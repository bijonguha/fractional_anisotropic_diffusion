...
for i = 1:niter
 ...
 % fprintf('\rIteration %d',i);
  ...
  %MAD implementation
  deln_med = medfilt2(deltaN,[3,3],'symmetric');
...
%   delS_M = 1.4826* medfilt2(dels_abs,[3,3],'symmetric') * sqrt(5);
  ...
  delw_med = medfilt2(deltaW,[3,3],'symmetric');
...
%   delE_M = 1.4826* medfilt2(dele_abs,[3,3],'symmetric') * sqrt(5);
  ...
  elseif option == 3
    ...
    cN(indN) =  1 * (1 - ((deltaN(indN)./delN_M(indN)).^2)).^2;
    cS(indS) =  1 * (1 - ((deltaS(indS)./delS_M(indS)).^2)).^2; 
    cW(indW) =  1 * (1 - ((deltaW(indW)./delW_M(indW)).^2)).^2;
    cE(indE) =  1 * (1 - ((deltaE(indE)./delE_M(indE)).^2)).^2;
